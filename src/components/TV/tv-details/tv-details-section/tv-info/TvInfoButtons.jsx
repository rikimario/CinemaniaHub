...
export default function TvInfoButtons({ user, series }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (user) {
        ...
        if (watchlistData) {
          const isInWatchlist =
            watchlistData.find((item) => item.id === series.id) !== undefined;
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (user) {
        ...
        if (watchedData) {
          const isInWatched =
            watchedData.find((item) => item.id === series.id) !== undefined;
  ...
  const handleAddToWatchlist = async () => {
    if (user) {
      const success = await listServices.addToWatchlist(user.email, series);
      if (success) {
  ...
  const handleAddToWatched = async () => {
    if (user) {
      const success = await listServices.addToWatched(user.email, series);
      if (success) {
        setIsInWatched(true);
